<script lang="ts">
    ...
    function getIndexCascadeStartOffsetPx() {
        ...
        return Math.max(INDEX_LEFT_CASCADE_MIN_OFFSET, Math.round(window.innerWidth * INDEX_LEFT_CASCADE_VIEWPORT_RATIO));
    }
...
{#if activeSection !== 'footer' && (currentSlide >= 1 || topLeftTravelVisible)}
    ...
    >
        <div 
            class="zalando"
            style="
                font-size: {topLeftMorphStage === 'full' ? `${TOP_LEFT_FONT_FULL}px` : `${TOP_LEFT_FONT_FINAL}px`};
                line-height: 1em;
                letter-spacing: {topLeftMorphStage === 'full' ? `${TOP_LEFT_LETTER_SPACING_FULL}em` : `${TOP_LEFT_LETTER_SPACING_FINAL}em`};
                font-weight: {topLeftTitleWeight};
                color: {topLeftMorphStage === 'full' ? 'white' : 'var(--color-blue)'};
                filter: {topLeftMorphStage === 'full' ? 'none' : 'drop-shadow(0 2px 8px rgba(25, 0, 255, 0.3))'};
                white-space: nowrap;
        ...
        >
            ...
            {:else}
                <span class="tj-mark"><span class="tj-initials">TJ</span><span class="tj-suffix" class:visible={topLeftMorphStage === 'final'}>25</span></span>
            {/if}
        </div>
    </div>
{/if}
...
<style>
    ...
    .hero-name {
        ...
        transition: opacity 0.4s ease, top 0.7s cubic-bezier(0.65, 0, 0.35, 1), left 0.7s cubic-bezier(0.65, 0, 0.35, 1), transform 0.7s cubic-bezier(0.65, 0, 0.35, 1), font-weight 0.52s cubic-bezier(0.65, 0, 0.35, 1);
    }
