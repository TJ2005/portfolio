<script lang="ts">
	import { deviceTilt, startDeviceTilt, type DeviceTiltState } from '$lib/deviceTilt';
	...
	function animate() {
		// Smooth cursor/gyro follow with lag
	...
	onMount(() => {
		...
		const unsubscribe = deviceTilt.subscribe((value) => {
			tilt = value;
			if (!isMobileViewport() || !value.permissionGranted) return;

			mouseX = value.x * CURSOR_INTENSITY;
			mouseY = value.y * CURSOR_INTENSITY;
		});
	...
	$effect(() => {
		if (!tilt.permissionGranted && typeof window !== 'undefined' && isMobileViewport()) {
			mouseX = 0;
			mouseY = 0;
		}
	});
