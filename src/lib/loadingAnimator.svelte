<script lang="ts">
	...
	let index = 0;
	let roller: ReturnType<typeof setInterval> | undefined;
	...
	onDestroy(() => {
		if (roller) clearInterval(roller);
	});
	const defaults = {
		gridSize: 3,
		brightness: 20,
	...
	let cellValues: Float32Array | undefined;

	function setupCanvasDimensions(width: number, height: number) {
		const minHeight = 100;
		let newWidth = width;
	...
	function generateHalftone(
		...
		const targetWidth = isExport
			? Math.max(previewWidth * scaleFactor, 1920)
			: previewWidth * scaleFactor;
		const targetHeight = isExport
			? Math.max(previewHeight * scaleFactor, 1080)
	...
	onMount(() => {
		halftoneCanvas = document.getElementById('halftoneCanvas') as HTMLCanvasElement | null;
...
<div class="w-full">
	<main class="flex min-h-screen flex-col items-center justify-center px-4">
		<canvas id="halftoneCanvas" class="h-auto max-w-full"></canvas>
		{#key index}
			<h1 class="mondwest -mt-8 text-center text-[var(--color-blue)]" transition:slide>
				{greetings[index]}
			</h1>
		{/key}
	</main>
